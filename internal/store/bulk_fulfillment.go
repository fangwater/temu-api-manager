		  ...
		  AND NOT EXISTS (
			...
			WHERE manual.parent_order_sn=o.parent_order_sn
			  AND ((manual.status='resolved' AND manual.outcome<>'') OR
			       (manual.active AND (manual.status<>'approved' OR manual.reasons && ARRAY['sku_unbound','inventory_rule','inventory_scope_incomplete','warehouse_sku_spec_incomplete','delivery_address_unsupported']::text[])))
		  )
